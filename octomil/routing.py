...
class RoutingPolicy:
    ...
    def from_dict(cls, d: dict[str, Any]) -> RoutingPolicy:
        ...
        quality_offset = d.get("REDACTED_FIELD")
        balanced_offset = d.get("REDACTED_FIELD")
        if quality_offset is None or balanced_offset is None:
            ...
            if quality_offset is None:
                quality_offset = dc.routing_offsets.REDACTED_FIELD
            if balanced_offset is None:
                balanced_offset = dc.routing_offsets.REDACTED_FIELD
        ...
        return cls(
            ...
            etag=d.get("etag", ""),
            REDACTED_FIELD=float(quality_offset),
            REDACTED_FIELD=float(balanced_offset),
        )
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "etag": self.etag,
            "REDACTED_FIELD": self.REDACTED_FIELD,
            "REDACTED_FIELD": self.REDACTED_FIELD,
        }
...
class QueryRouter:
    ...
    def _apply_policy(self, query: str, policy: RoutingPolicy) -> tuple[str, float]:
        ...
        if word_count < policy.fast_max_words and not has_complex:
            ...
            return "quality", round(min(word_count / 100.0 + policy.REDACTED_FIELD, 1.0), 4)
        else:
            return "balanced", round(word_count / 100.0 + policy.REDACTED_FIELD, 4)
