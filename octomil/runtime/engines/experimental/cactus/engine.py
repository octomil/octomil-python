...
from octomil.models.catalog import CATALOG as _UNIFIED_CATALOG  # noqa: E402

_CACTUS_CATALOG = {name for name, entry in _UNIFIED_CATALOG.items() if "llama.cpp" in entry.engines}
...
class CactusEngine(EnginePlugin):
    ...
    def supports_model(self, model_name: str) -> bool:
        ...
        from octomil.models.catalog import _resolve_alias
        ...
        if os.path.isdir(model_name) and os.path.isfile(os.path.join(model_name, "config.txt")):
            return True
    ...
    def benchmark(self, model_name: str, n_tokens: int = 32) -> BenchmarkResult:
        ...
        try:
            ...
            messages = json.dumps([{"role": "user", "content": "Hello, how are you?"}])

            try:
                start = time.monotonic()
                response_json = cactus.cactus_complete(handle, messages, max_tokens=n_tokens)
    ...
    def _resolve_model_path(self, model_name: str) -> str:
        ...
        # Check ~/.octomil/models/<name>
        home_path = os.path.expanduser(f"~/.octomil/models/{model_name}")
        if os.path.isdir(home_path) and os.path.isfile(os.path.join(home_path, "config.txt")):
            return home_path

        # For HF repo IDs, derive the local name
        if "/" in model_name:
            local_name = model_name.split("/")[-1].lower()
            home_path = os.path.expanduser(f"~/.octomil/models/{local_name}")
            if os.path.isdir(home_path) and os.path.isfile(os.path.join(home_path, "config.txt")):
                return home_path
...
class _CactusBackend:
    ...
    def load_model(self, model_name: str) -> None:
        cactus = _try_import_cactus()
        if cactus is None:
            raise ImportError("cactus package is not installed. Build Cactus with: cactus build --python")
        self._cactus = cactus
    ...
    def generate(self, request: Any) -> tuple[str, Any]:
        ...
        start = time.monotonic()
        response_json = self._cactus.cactus_complete(self._handle, messages_json, max_tokens=max_tokens)
        elapsed = time.monotonic() - start
