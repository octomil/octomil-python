...
_GGUF_CATALOG = {name for name, entry in _UNIFIED_CATALOG.items() if "llama.cpp" in entry.engines}

# MoE models in the catalog that llama.cpp supports natively
_MOE_MODELS = {
    name for name, entry in _UNIFIED_CATALOG.items() if entry.architecture == "moe" and "llama.cpp" in entry.engines
}
...
class LlamaCppEngine(EnginePlugin):
    ...
    def _has_cuda() -> bool:
        try:
            from llama_cpp import Llama  # type: ignore[import-untyped]  # noqa: F401
    ...
    def create_backend(self, model_name: str, **kwargs: Any) -> Any:
        ...
        if self.is_moe_model(model_name):
            logger.info(
                "MoE model '%s' detected — llama.cpp handles expert routing natively via GGUF",
                model_name,
            )
