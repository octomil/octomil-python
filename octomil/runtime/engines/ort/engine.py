...
_ORT_CATALOG = {name for name, entry in _UNIFIED_CATALOG.items() if "onnxruntime" in entry.engines}
...
class ONNXRuntimeEngine(EnginePlugin):
    ...
    def supports_model(self, model_name: str) -> bool:
        ...
        return canonical in _ORT_CATALOG or model_name.endswith(".onnx") or "/" in model_name
    ...
    def benchmark(self, model_name: str, n_tokens: int = 32) -> BenchmarkResult:
        ...
        return BenchmarkResult(engine_name=self.name, error="onnxruntime not available")

    def _benchmark_genai(self, model_name: str, n_tokens: int) -> BenchmarkResult:
        try:
    ...
    def _benchmark_session(self, model_name: str, n_tokens: int) -> BenchmarkResult:
        try:
            import numpy as np
            import onnxruntime as ort  # type: ignore[import-untyped]
    ...
    def _resolve_model_path(self, model_name: str) -> str:
        ...
        candidates = [
            ...
            os.path.expanduser(f"~/.octomil/models/{model_name}/model.onnx"),
        ]
...
class _ORTBackend:
    ...
    def load_model(self, model_name: str) -> None:
        ...
        if _has_onnxruntime_genai():
            try:
                ...
                logger.info("Loaded %s with ONNX Runtime GenAI", model_name)
                return
            except Exception as exc:
                logger.debug(
    ...
    def _generate_genai(self, prompt: str, max_tokens: int) -> tuple[str, Any]:
        import onnxruntime_genai as og  # type: ignore[import-untyped]
    ...
    def _generate_session(self, prompt: str, max_tokens: int) -> tuple[str, Any]:
        import numpy as np
    ...
    async def generate_stream(
        ...
        from octomil.serve import GenerationChunk
