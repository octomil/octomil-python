"""
Device auth client for bootstrap/refresh/revoke token flow.
"""
...
class DeviceAuthClient:
    """
    Device auth manager with secure token persistence using system keyring.
    """
    ...
    def __init__(
        ...
        *,
        ...
        if keyring is None:
            raise RuntimeError("keyring package is required for secure token storage")
    ...
    async def refresh(self) -> DeviceTokenState:
        ...
        if not state:
            raise RuntimeError("No device token state found in keyring")
