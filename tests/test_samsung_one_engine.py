...
from octomil.runtime.engines.experimental.samsung_one.engine import (
    SamsungOneEngine,
    _get_onert_version,
    _has_onert,
    _select_backend,
...
class TestSamsungOneEngine:
    ...
    def test_detect_with_onert(self) -> None:
        with patch("octomil.runtime.engines.experimental.samsung_one.engine._has_onert", return_value=True):
            assert self.engine.detect() is True

    def test_detect_without_onert(self) -> None:
        with patch("octomil.runtime.engines.experimental.samsung_one.engine._has_onert", return_value=False):
            assert self.engine.detect() is False

    def test_detect_info_available(self) -> None:
        with (
            patch("octomil.runtime.engines.experimental.samsung_one.engine._has_onert", return_value=True),
            patch(
                "octomil.runtime.engines.experimental.samsung_one.engine._get_onert_version",
                return_value="1.31.0",
            ),
        ):
    ...
    def test_benchmark_with_onert(self) -> None:
        ...
        with (
            patch("octomil.runtime.engines.experimental.samsung_one.engine._has_onert", return_value=True),
            patch(
                "octomil.runtime.engines.experimental.samsung_one.engine.SamsungOneEngine._resolve_model_path",
