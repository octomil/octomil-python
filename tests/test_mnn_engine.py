...
from octomil.runtime.engines.experimental.mnn.engine import MNNEngine, _parse_tps_from_output
...
class TestMNNEngine:
    ...
    def test_display_name_macos(self) -> None:
        with patch("octomil.runtime.engines.experimental.mnn.engine.platform.system", return_value="Darwin"):
            assert "Metal" in self.engine.display_name

    def test_display_name_linux(self) -> None:
        with patch("octomil.runtime.engines.experimental.mnn.engine.platform.system", return_value="Linux"):
            assert "Vulkan" in self.engine.display_name

    def test_display_name_windows(self) -> None:
        with patch("octomil.runtime.engines.experimental.mnn.engine.platform.system", return_value="Windows"):
    ...
    def test_detect_with_pymnn(self) -> None:
        with patch("octomil.runtime.engines.experimental.mnn.engine._has_pymnn", return_value=True):
            assert self.engine.detect() is True

    def test_detect_with_cli(self) -> None:
        with (
            patch("octomil.runtime.engines.experimental.mnn.engine._has_pymnn", return_value=False),
            patch(
                "octomil.runtime.engines.experimental.mnn.engine._find_mnn_cli",
    ...
    def test_detect_nothing_available(self) -> None:
        with (
            patch("octomil.runtime.engines.experimental.mnn.engine._has_pymnn", return_value=False),
            patch("octomil.runtime.engines.experimental.mnn.engine._find_mnn_cli", return_value=None),
        ):
            assert self.engine.detect() is False

    def test_detect_info(self) -> None:
        with (
            patch("octomil.runtime.engines.experimental.mnn.engine._has_pymnn", return_value=True),
            patch(
                "octomil.runtime.engines.experimental.mnn.engine._find_mnn_cli",
                return_value="/usr/bin/mnn-llm",
            ),
            patch("octomil.runtime.engines.experimental.mnn.engine.platform.system", return_value="Darwin"),
        ):
    ...
    def test_benchmark_python(self) -> None:
        ...
        with (
            patch("octomil.runtime.engines.experimental.mnn.engine._has_pymnn", return_value=True),
            patch("octomil.runtime.engines.experimental.mnn.engine.MNNEngine._benchmark_python") as mock_bench,
        ):
    ...
    def test_benchmark_cli(self) -> None:
        with (
            ...
            patch("octomil.runtime.engines.experimental.mnn.engine.MNNEngine._benchmark_cli") as mock_bench,
        ):
    ...
    def test_create_backend_without_pymnn(self) -> None:
        with patch("octomil.runtime.engines.experimental.mnn.engine._has_pymnn", return_value=False):
