...
from octomil.device_agent.artifact_downloader import ArtifactDownloader, CHUNK_SIZE
...
class TestStartDownload:
    def test_creates_chunks(self, setup) -> None:
        ...
        rows = db.execute(
            "SELECT * FROM download_chunks WHERE artifact_id = 'a1' ORDER BY chunk_index"
        )
        assert len(rows) == 3
...
class TestResumeDownload:
    ...
    def test_excludes_complete_chunks(self, setup) -> None:
        ...
        dl.start_download("a1", manifest, "https://example.com")
        db.execute(
            "UPDATE download_chunks SET status = 'COMPLETE' "
            "WHERE artifact_id = 'a1' AND chunk_index = 0"
        )
        pending = dl.resume_download("a1")
